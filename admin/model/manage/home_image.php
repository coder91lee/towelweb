...
class ModelManageHomeImage extends Model {
	...
	public  function addHomeImage($data,$image){
		$this->db->query(" INSERT into " . DB_DATABASE . ".home_image set image = '" .$this->db->escape($image).
						  "', link = '" . $this->db->escape($data['link']) .
						  "', name = '" . $this->db->escape($data['name']) ."'");
	}
	
   public  function editHomeImage($id,$data,$image){
		$this->db->query(" UPDATE " . DB_DATABASE . ".home_image set image = '" .$this->db->escape($image).
						  "', link = '" . $this->db->escape($data['link']).
						  "', name = '" . $this->db->escape($data['name']) .
	...
	public function getHomeImageById($id) {
		$query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".home_image WHERE home_image_id = '" . (int)$id . "'");

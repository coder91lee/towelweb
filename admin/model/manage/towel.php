...
class ModelManageTowel extends Model {
	...
	public  function addTowel($data,$towel_image){
		$this->db->query(" INSERT into " . DB_DATABASE . ".towel set towel_name = '" . $this->db->escape($data['towel_name']).
   ...
   public  function editTowel($id,$data,$towel_image){
		$this->db->query(" UPDATE " . DB_DATABASE . ".towel set towel_name = '" . $this->db->escape($data['towel_name']).
						  "',  towel_image = '" .$this->db->escape($towel_image).
						  "',  towel_cate_id = " .$this->db->escape($data['towel_cate_id']).
						  ", status = " . (int)$data['status'].
	...
	public function getTowelById($id) {
		$query = $this->db->query("SELECT * FROM " . DB_DATABASE . ".towel WHERE towel_id = '" . (int)$id . "'");
	
		return $query->row;
	}
	
	public function increaseCountApp($towel_id){
	    $sql = "update " . DB_DATABASE . ".towel a set count_app = count_app + 1 where a.towel_id = $towel_id";

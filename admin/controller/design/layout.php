...
class ControllerDesignLayout extends Controller {
	...
	public function insert() {
      ...
      public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//Xoa du lieu trong layout cu
			$this -> model_design_layout -> deleteModuleInLayout($this->request->get['layout_id']);
		   
			//=========== CONTENT TOP ===============================================================
			$modules_in_layout = $this->request->post['modules_top'];
			$count = 1;
			$pos = 1;
			foreach ($modules_in_layout as $module_id)
			{
				$this -> model_design_layout -> addModuleInLayout($this->request->get['layout_id'], $module_id, $count, $pos, $web_version);
				$count ++;
			}
			
			//=========== CONTENT LEFT ===============================================================
			$modules_in_layout = $this->request->post['modules_left'];
			$count = 1;
			$pos = 2;
			foreach ($modules_in_layout as $module_id)
			{
			...
			//=========== CONTENT BODY ===============================================================
			$modules_in_layout = $this->request->post['modules_body'];
			$count = 1;
			$pos = 3;
			foreach ($modules_in_layout as $module_id)
			{
			...
			//=========== CONTENT RIGHT ===============================================================
			$modules_in_layout = $this->request->post['modules_right'];
			$count = 1;
			$pos = 4;
			foreach ($modules_in_layout as $module_id)
			{
			...
			//=========== CONTENT BOTTOM ===============================================================
			$modules_in_layout = $this->request->post['modules_bottom'];
			$count = 1;
			$pos = 5;
			foreach ($modules_in_layout as $module_id)
			{
	...
	private function getForm() {
		...
		if (isset($this->request->post['layout_route'])) {
			...
			$layout_route = $this->model_design_layout->getLayoutRoute($this->request->get['layout_id']);
			
			// get modules in layout
			$modules_in_layout = $this->model_design_layout->getLayoutModule($this->request->get['layout_id'], 1); //pc
			
			$this -> data['modules_in_layout'] = $modules_in_layout;
			
			
			$this->data['layout_route'] = $layout_route;
			//get all modules
			$this->load->model('extension/module');
			$extensions = $this -> model_extension_module -> getAllModulesNoLimit();
